import type { Province, District, Ward, AddressMapping, ConversionResult } from '../types/address';
...
export class AddressMapper {
  ...
  convertOldToNew(oldProvinceId: string): ConversionResult | null {
    ...
    if (!provinceMapping) {
      ...
      if (oldProvince && newProvince) {
        return {
          oldAddress: oldProvince,
          newAddress: newProvince,
  ...
  convertNewToOld(newProvinceId: string): ConversionResult[] {
    ...
    return provinceMapping.oldIds.map(oldId => {
